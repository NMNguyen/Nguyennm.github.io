...
function search_position(id, mode = 'tree') {
    ...
    if (name) {
        ...
        if (mode === 'tree') {
            // reset tree
            $("#org-chart").html('');
            dataSource.children = []; // clear dataSource.children đối với trường hợp node cấp dưới CEO
            data_node = {}; // clear data_node đối với trường hợp các node cháu chít ....
            init();
        }
        setTimeout(function () {
            st.graph.eachNode(function (node) {
                if (node.data.id == name) {
                    ...
                    // st.onClick(node.id);
                    setTimeout(function () {
                       PositionApp.clickOneNode(node, false);
                    }, 1500);
    ...
    if (node_search_list.length) {
        ...
        if (node_search_list.length == 0 && found_node_id) {
            ...
            auto_timeout_id = setInterval(function () {
                if (!st.busy) {
                    st.onClick(found_node_id);
                    init_node(st.graph.getNode(found_node_id));
                    PositionApp.loading = false;
